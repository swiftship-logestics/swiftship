...
<template>
  <div class="grid w-full gap-4 lg:grid-cols-2">
    <div class="w-full">
      ...
      <input id="first-name" v-model="billing.firstName" placeholder="Enter first name" autocomplete="given-name" type="text" required />
    </div>

    <div class="w-full">
      <label for="last-name">{{ $t('messages.billing.lastName') }}</label>
      <input id="last-name" v-model="billing.lastName" placeholder="Enter last name" autocomplete="family-name" type="text" />
    </div>

    <div v-if="isBillingAddressEnabled" class="w-full col-span-full">
      <label for="address1">{{ $t('messages.billing.address1') }}</label>
      <input id="address1" v-model="billing.address1" placeholder="Enter address 1" autocomplete="street-address" type="text" required />
    </div>

    <div v-if="isBillingAddressEnabled" class="w-full col-span-full">
      <label for="address2">{{ $t('messages.billing.address2') }} ({{ $t('messages.general.optional') }})</label>
      <input id="address2" v-model="billing.address2" placeholder="Enter address 2" autocomplete="address-line2" type="text" />
    </div>

    <div v-if="isBillingAddressEnabled" class="w-full">
      <label for="city">{{ $t('messages.billing.city') }}</label>
      <input id="city" v-model="billing.city" placeholder="Enter city" autocomplete="locality" type="text" required />
    </div>
    ...
    <div v-if="isBillingAddressEnabled" class="w-full">
      ...
      <input id="zip" v-model="billing.postcode" placeholder="Enter postal code" autocomplete="postal-code" type="text" required />
    </div>

    <div v-if="isBillingAddressEnabled" class="w-full col-span-full">
      <label for="phone">{{ $t('messages.billing.phone') }}
         <!-- ({{ $t('messages.general.optional') }}) -->
        </label>
      <input id="phone" v-model="billing.phone" placeholder="Enter number" autocomplete="tel" type="tel" required />
    </div>
  </div>
</template>
